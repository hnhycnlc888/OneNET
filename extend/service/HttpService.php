...
class HttpService
{
    ...
    public static function get($url, $data = [], $second = 30, $header = [])
    {
        ...
        curl_close($curl);
		//记录日志
		App::$debug && Log::record('[ API_URL ] ' . var_export($url, true), 'info');
		App::$debug && Log::record('[ API_HEADER ] ' . var_export($header, true), 'info');
		App::$debug && Log::record('[ API_PARAM ] ' . var_export($data, true), 'info');
		App::$debug && Log::record('[ API_RESULT ] ' . var_export($content, true), 'info');
		App::$debug && Log::record('[ API_HTTP_INFO ] ' . var_export($status, true), 'info');
