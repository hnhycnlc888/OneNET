...
define(['jquery', 'admin.plugs'], function () {
    ...
    this.$body.on('click', '[data-update]', function () {
        ...
        $.msg.confirm('确定要操作数据吗？', function () {
            $.form.load(action, {field: field, value: value, id: id}, 'POST');
        });
    });

    /*! 注册 data-request 事件行为 */
    this.$body.on('click', '[data-request]', function () {
		var request = $(this).attr('data-request');
		var operate = $(this).attr('data-operate');
        var action = $(this).attr('data-action') || $(this).parents('[data-location]').attr('data-location');
        var project_id = $(this).attr('data-project_id');
        var product_id = $(this).attr('data-product_id');
        var device_name = $(this).attr('data-device_name');
		if(request === 'onenet' && operate === 'reboot'){
			$.msg.confirm('确定要重启操作吗？', function () {
				$.form.load(action, {project_id: project_id, product_id: product_id, device_name: device_name}, 'POST');
			});
		}
    });
